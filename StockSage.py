...
start = st.sidebar.date_input('Start', datetime.date(2023, 1, 1))  # start date input
...
# Real-Time Stock Price Section Starts Here
    ...
    if (
        ...
        if a == "":  # if user doesn't select any company
            ...
            dropdown1 = st.selectbox('Pick your chart', chart)  # dropdown for selecting chart type
            with st.spinner('Loading...'):  # spinner while loading
                time.sleep(2)
...
# Stock Price Prediction Section Starts Here
    ...
    if a == "":  # if user doesn't select any company
        ...
        data = yf.download(symb_list, start=start, end=end)  # download data from yfinance
        data.reset_index(inplace=True)  # reset index
        st.subheader('Raw Data of {}'.format(a))  # display raw data
        st.write(data)  # display data
        ...
        plot_raw_data()  # plot raw data
        n_years = st.slider('Years of prediction:', 1, 4)  # slider for selecting number of years
        period = n_years * 365  # calculate number of days

        df_train = data[['Date', 'Close']]  # create dataframe for training data
...
# Stock Price Prediction Section Ends Here
    ...
    st.markdown('<p class="big-font">StockSage is a web application that allows users to visualize Stock Performance Comparison, Real-Time Stock Prices and Stock Price Prediction. This application is developed using Streamlit. Streamlit is an open source app framework in Python language. It helps users to create web apps for Data Science and Machine Learning in a short time. This Project is developed by NawinSRIvatsav. You can find more about the developers on their GitHub Profiles shared below.<br>Hope you are able to employ this application well and get your desired output.<br> Cheers!</p>', unsafe_allow_html=True)
    st.subheader('NawinSRIvatsav [![Repo](https://badgen.net/badge/icon/GitHub?icon=github&label)](https://github.com/NawinSRIvatsav/NawinSRIvatsav.git)')
